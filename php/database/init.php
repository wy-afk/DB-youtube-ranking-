...
$conn_init->query(str_replace("_youtube_trending_data", "CREATE TABLE user_upload", $sql_fmt[1])); // user upload
...
// Load data
$sql_fmt = [ // XX_youtube_trending_data: basic data
	"LOAD DATA LOCAL INFILE './dataset/",
    "_youtube_trending_data.csv' INTO TABLE ",
    "_youtube_trending_data FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\"' IGNORE 1 ROWS"
];
...
$sql_fmt = [ // XX_youtube_trending_data: country_id
    "UPDATE ",
    "_youtube_trending_data SET country_id = '",
    "'"
];

foreach($country_IDs as $cID){ // country: rows
	$sql = $sql_fmt[0] . $cID . $sql_fmt[1] . $cID . $sql_fmt[2];
	$conn_init->query($sql);
}
